<?php
/*
Template name: Карточка авто
*/

// Подключаем Модуль
use aea\Wp_Car_Auction_Lite\Wp_Car_Auction_Plugin_Lite;

// Проверяем, если это запрос детальной страницы автомобиля из плагина Car Auction
global $car_auction_detail_data;
$is_car_auction_detail = !empty($car_auction_detail_data);

// Также проверяем прямо по query var
if (!$is_car_auction_detail && get_query_var('car_auction_detail')) {
    $is_car_auction_detail = true;

    // Создаем данные из query vars если они не были переданы глобально
    ...
    // Конвертируем market_name в market code
    $market_name_to_code = array(
        'japan' => 'main',
        'korea' => 'korea',
        'china' => 'china',
		'bike' => 'bike',
    ...
    $car_auction_detail_data['market'] = $market_name_to_code[$car_auction_detail_data['market_name']] ?? 'main';

    if (get_option('car_auction_debug_mode', false)) {
        error_log('single-auto.php: Created car_auction_detail_data from query vars: ' . print_r($car_auction_detail_data, true));
    }
}

if ($is_car_auction_detail) {
    // Отладочная информация
    if (get_option('car_auction_debug_mode', false)) {
        error_log('single-auto.php: Processing car auction detail page');
        error_log('single-auto.php: car_auction_detail_data = ' . print_r($car_auction_detail_data, true));
    }

    // Обрабатываем как детальную страницу автомобиля из плагина
    $car_id = $car_auction_detail_data['car_id'];
    $market = $car_auction_detail_data['market'];
    $market_name = $car_auction_detail_data['market_name'];

    // Получаем данные автомобиля через плагин
    if (class_exists('aea\Wp_Car_Auction_Lite\core\Car_Auction_Search')) {
        /*
        $search_instance = Wp_Car_Auction_Plugin_Lite::getCarAuctionSearchSync();
        $car_data = $search_instance->get_car_data_for_theme($car_id, $market);

        if (!$car_data) {
            // Показываем 404 если автомобиль не найден
            if (get_option('car_auction_debug_mode', false)) {
                error_log('single-auto.php: Car data not found for ID: ' . $car_id . ' in market: ' . $market);
            }
            global $wp_query;
            $wp_query->set_404();
            status_header(404);
        ...
        // Отображаем детальную страницу автомобиля с данными из API
        if (get_option('car_auction_debug_mode', false)) {
            error_log('single-auto.php: Displaying car detail shortcode for car: ' . $car_id);
        }
        */

        echo do_shortcode('[car_auction_detail car_id="' . esc_attr($car_id) . '" market="' . esc_attr($market) . '"]');
        exit; // Use exit instead of return to prevent further output
    } else {
        if (get_option('car_auction_debug_mode', false)) {
            error_log('single-auto.php: Car_Auction_Search class not found');
        }
    }
}

// Если это не детальная страница из плагина, продолжаем обычную обработку
        ...
        <div class="car-hero-wrapper">
            <div class="car-main-wrapper">
                <div class="car-main-left">
                    <div class="bc-wrapper">
                        <div class="breadcrumbs w-richtext">
                            <p id="breacrumbs"><?php echo breadcrumbs() ?></p>
                        </div>
                    </div>
                    <div class="swiper swiper_main">
                        <?php if( have_rows('fotografii') ){ ?><div class="swiper-wrapper"><?php global $parent_id; if(isset($loop_id)) $parent_id = $loop_id; $loop_index = 0; $loop_title="Фотографии"; $loop_field = "fotografii"; while( have_rows('fotografii') ){ global $loop_id; $loop_index++; $loop_id++; the_row(); ?>
                            <div class="swiper-slide prod"><a href="#" class="lightbox-link w-inline-block w-lightbox"><img src="<?php $field = get_sub_field('foto'); if(isset($field['url'])){ echo($field['url']); }elseif(is_numeric($field)){ echo(wp_get_attachment_image_url($field, 'full')); }else{ echo($field); } ?>" loading="lazy" alt="<?php echo !empty($field['alt']) ? esc_attr($field['alt']) : ''; ?>" class="image"><script type="application/json" class="w-json"><?php
                    ...
                    <div class="sw-thumbs-wrapper">
                        <div class="swiper swiper_thumbnail">
                            <?php if( have_rows('fotografii') ){ ?><div class="swiper-wrapper thumbs"><?php global $parent_id; if(isset($loop_id)) $parent_id = $loop_id; $loop_index = 0; $loop_title="Фотографии"; $loop_field = "fotografii"; while( have_rows('fotografii') ){ global $loop_id; $loop_index++; $loop_id++; the_row(); ?>
                                <div class="swiper-slide thumb"><img src="<?php $field = get_sub_field('foto'); if(isset($field['url'])){ echo($field['url']); }elseif(is_numeric($field)){ echo(wp_get_attachment_image_url($field, 'full')); }else{ echo($field); } ?>" loading="lazy" alt="<?php echo !empty($field['alt']) ? esc_attr($field['alt']) : ''; ?>" class="image"></div>
                ...
                <div class="car-main-right">
                    <h1 class="h1 left"><span><?php $terms = get_the_terms(get_the_ID(), "brand"); if( is_array($terms) ) echo($terms[0]->name); ?></span><span class="model"><?php echo get_field('model') ?></span> <span class="body-part"><?php echo get_field('kuzov') ?></span></h1>
                    <div class="car-main-top">
                        <div class="m-16-600 gray">Основные характеристики:</div>
                        <div class="car-chars">
                            <?php if(get_field('kuzov')): ?>
                                <div id="w-node-d0a1dae4-7223-d46c-1d7b-cf92eac304f0-dbce63b7" class="one-char">
                                    <div class="one-char-left">
                                        <div class="m-16-400">Кузов</div>
                                    </div>
                                    <div class="one-char-right">
                                        <div class="m-16-600"><?php echo get_field('kuzov') ?></div>
                                    </div>
                                </div>
                            <?php endif; ?>
                            <?php if(get_field('car_year')): ?>
                                <div class="one-char">
                                    ...
                                    <div class="one-char-right">
                                        <div class="m-16-600"><?php echo get_field('car_year') ?></div>
                                    </div>
                                </div>
                            <?php endif; ?>
                            <?php if(get_field('car_probeg')): ?>
                                <div class="one-char">
                                    <div class="one-char-left">
                                        <div class="m-16-400">Пробег</div>
                                    </div>
                                    <div class="one-char-right">
                                        <div class="m-16-600"><?php echo get_field('car_probeg') ?></div>
                                    </div>
                                </div>
                            <?php endif; ?>
                            <?php if(get_field('obem_dvigatelya')): ?>
                                <div class="one-char">
                                    <div class="one-char-left">
                                        <div class="m-16-400">Объем Двигателя</div>
                                    </div>
                                    <div class="one-char-right">
                                        <div class="m-16-600"><?php echo get_field('obem_dvigatelya') ?></div>
                                    </div>
                                </div>
                            <?php endif; ?>
                            <?php if(get_field('transmissiya')): ?>
                                <div class="one-char">
                                    <div class="one-char-left">
                                        <div class="m-16-400">Трансмиссия</div>
                                    </div>
                                    <div class="one-char-right">
                                        <div class="m-16-600"><?php echo get_field('transmissiya') ?></div>
                                    </div>
                                </div>
                            <?php endif; ?>
                            <?php if(get_field('tip_topliva')): ?>
                                <div class="one-char">
                                    <div class="one-char-left">
                                        <div class="m-16-400">Топливо</div>
                                    </div>
                                    <div class="one-char-right">
                                        <div class="m-16-600"><?php echo get_field('tip_topliva') ?></div>
                                    </div>
                                </div>
                            <?php endif; ?>
                            <?php if(get_field('privod')): ?>
                                <div class="one-char">
                                    <div class="one-char-left">
                                        <div class="m-16-400">Привод</div>
                                    </div>
                                    <div class="one-char-right">
                                        <div class="m-16-600"><?php echo get_field('privod') ?></div>
                                    </div>
                                </div>
                            <?php endif; ?>
                            <?php if(get_field('ocenka_aukciona')): ?>
                                <div class="one-char">
                                    <div class="one-char-left">
                                        <div class="m-16-400">Оценка аукциона</div>
                                    </div>
                                    <div class="one-char-right">
                                        <div class="m-16-600"><?php echo get_field('ocenka_aukciona') ?></div>
                                    </div>
                                </div>
                            <?php endif; ?>
                        </div>
                        <div class="price-car-wrapper">
                            ...
                            <div class="m-32-600"><?php
                                echo get_field('stoimost_avto') ?: '—';
                            ?></div>
                        </div>
                    ...
                    <div class="car-buttons-wrapper"><a data-w-id="834630e3-7369-7641-8907-b2afa64b82d0" href="#" class="button-red w-button">Заказать этот автомобиль</a><a data-w-id="2aeff7e9-ef9d-123f-ae35-eda71b29fa04" href="#" class="button-light-gray w-button">Получить консультацию</a></div>
...
<?php
$car_id_meta = get_post_meta(get_the_ID(), '_car_auction_id');
$car_id = !empty($car_id_meta) ? array_shift($car_id_meta) : '';
$market_meta = get_post_meta(get_the_ID(), '_car_auction_market');
$market_name = !empty($market_meta) ? array_shift($market_meta) : '';

if($market_name == 'bike'): ?>

    <?php // У BIKE нестандартный формат информации! Временно скрыто ?>

<?php elseif ($market_name !== 'china'): ?>
    <?php if ( !empty( get_field('tamozhennyj_list') ) || have_rows('opcii') ): ?>
        <section class="section stdrt gray-new">
            <div class="container">
                <div class="h2-wrapper">
                    <h2 class="h2"><?php echo get_field('tamozhennyj_list') ? 'Аукционный лист' : 'Комплектация'; ?></h2>
                </div>

                <?php if (get_field('tamozhennyj_list')): ?>
                    <div class="auc_list_section">
                        <div class="auc_img">
                            <?php
                            $tamozhennyj_list = get_field('tamozhennyj_list');
                            if (!empty($tamozhennyj_list[0]['foto']['url'])):
                                $image_url = $tamozhennyj_list[0]['foto']['url'];
                                $image_alt = !empty($tamozhennyj_list[0]['foto']['alt']) ? $tamozhennyj_list[0]['foto']['alt'] : 'Таможенный лист';
                                ?>
                ...
                <?php elseif (have_rows('opcii')): ?>
                    <div class="options-wrapper">
                        <?php
                        global $parent_id;
                        if (isset($loop_id)) $parent_id = $loop_id;
                        $loop_index = 0;
                        $loop_title = "Опции";
                        $loop_field = "opcii";
                        while (have_rows('opcii')):
                            global $loop_id;
                            $loop_index++;
                            $loop_id++;
                            the_row();
                            ...
                            if (!empty($tekst)):
                                ...
                                <div class="one-option-wrapper">
                                    ...
                                    <div class="m-16-600"><?php echo esc_html($tekst); ?></div>
                                </div>
                            <?php
                            endif;
                        endwhile;
...
$market = $market_name_to_code[$market_name] ?? 'main'; // Код маркета
$brand = is_array($brand_terms) ? $brand_terms[0]->name : ''; // Название марки
$model = get_field('model', $post_id); // Название модели

/*
echo Wp_Car_Auction_Plugin_Lite::getCarAuctionSearchSync()->render_similar_cars_section(
        $market,
        $brand,
        $model,
        'grid'
        ...
        if ($placeholder.length) {
            $.ajax({
                ...
                data: {
                    ...
                    car_id: '<?php echo esc_js($car_id); ?>',
                    market: '<?php echo esc_js($market); ?>',
    ...
    <div class="container">
        <div class="footer-wrapper">
            ...
            <div class="footer-col">
                ...
                <div class="footer-links-wrapper"><a href="/" class="m-14-400 white">Главная</a><a href="/japan/" class="m-14-400 white">Авто из Японии</a><a href="/korea/" class="m-14-400 white">Авто из Кореи</a><a href="/china/" class="m-14-400 white">Авто из Китая</a><a href="/about/" class="m-14-400 white">О нас</a><a href="/blog/" class="m-14-400 white">Статьи</a><a href="/contact/" class="m-14-400 white">Контакты</a></div>
            </div>
            <div class="footer-col">
...
<div class="lbox">
    ...
    <div class="form-lbox-wrapper">
        ...
        <div class="h-24-700 mid"><span><?php $terms = get_the_terms(get_the_ID(), "brand"); if( is_array($terms) ) echo($terms[0]->name); ?></span><span class="model"><?php echo get_field('model') ?></span></div>
        <div class="w-form">
...
<script type="text/javascript" src="<?php bloginfo('template_url'); ?>/js/single-auto.js?ver=1759315945"></script></body>
</html>
